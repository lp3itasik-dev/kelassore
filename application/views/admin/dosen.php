                      ...
                      foreach ($read as $r) { ?>
                        <tr>
                          ...
                          <td class="text-center">
                            <button class="btn btn-sm btn-warning" title="Edit" onclick="return ubah('<?= $r->id_dosen ?>', '<?= $r->dosen ?>')"><i class="fa fa-edit"></i></button>
                            <button class="btn btn-sm btn-danger" title="Hapus" onclick="return hapus('<?= $r->id_dosen ?>', '<?= $r->dosen ?>')"><i class="fa fa-trash"></i></button>
                          </td>
                        </tr>
                      <?php } ?>
                    </tbody>
...
<script>
    function tambah(){
      ...
      $('.modal-title').html('Tambah Dosen');
            ...
            $('#formId').attr('action', '<?= base_url() ?>dosen/add');
            $('[name="dosen"]').attr('required','');
            
        }
        function ubah(id_dosen,dosen){
      $('#myModal').modal({backdrop: 'static', keyboard: false});
      $('.modal-title').html('Ubah Dosen');
            $('[name="dosen"]').val(dosen);
            $('#submit').html('SIMPAN');
            $('.hapus').addClass('d-none');
            $('.input').removeClass('d-none');
            $('#formId').attr('action', '<?= base_url() ?>dosen/edit/'+id_dosen);
            $('[name="dosen"]').attr('required','');
        }
        function hapus(id_dosen,dosen){
      $('#myModal').modal({backdrop: 'static', keyboard: false});
      $('.modal-title').html('Hapus Dosen');
            $('#dosen').html(dosen);
            $('#submit').html('HAPUS');
            $('.input').addClass('d-none');
            $('.hapus').removeClass('d-none');
            $('#formId').attr('action', '<?= base_url() ?>dosen/delete/'+id_dosen);
            $('[name="dosen"]').removeAttr('required');
        }
  </script>
        ...
        <form id="formId" action="" method="POST">
          <div class="modal-body input">
            <div class="form-group">
              ...
              <input type="text" class="form-control" placeholder="Nama Dosen" name="dosen" required autocomplete="off">
            </div>
          </div>
          <div class="modal-body hapus h5">
            Anda yakin ingin menghapus kategori <b id="dosen"></b>?
          </div>
